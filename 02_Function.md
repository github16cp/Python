...
附录：[md语法|LaTex数学公式](https://blog.csdn.net/zryxh1/article/details/53161011)
##  调用函数
------
### 调用函数
------
...
def calc(*numbers):
    ...
    return sum
```
定义可变参数和定义一个list或tuple参数相比，仅仅在参数前面加了一个`*`号。在函数内部，参数`numbers`接收到的是一个`tuple`，因此，函数代码完全不变。但是，调用该函数时，可以传入任意个参数，包括`0`个参数：
```Python
>>> calc(1, 2)
5
>>> calc()
...
14
```
`*nums`表示把`nums`这个list的所有元素`作为可变参数`传进去。这种写法相当有用，而且很常见。
...
使用递归函数需要注意防止`栈溢出`。在计算机中，函数调用是通过栈（stack）这种数据结构实现的，每当进入一个函数调用，栈就会加一层栈帧，每当函数返回，栈就会减一层栈帧。由于栈的大小不是无限的，所以，递归调用的次数过多，会导致栈溢出。可以试试`fact(1000)`：
```Python
>>> fact(1000)
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
