...
def to_timestamp(dt_str, tz_str):
    #字符串转化为datatime类型
    dt_in = datetime.strptime(dt_str,'%Y-%m-%d %H:%M:%S')
    #获取时区
    tz = re.match(r'^UTC([+\-]\d{1,2}):00$',tz_str).group(1)
    #创建时区
    tz_utc = timezone(timedelta(hours = int(tz)))
    #强制设置时区,将datetime类型时间转化为utc时间
    td = dt_in.replace(tzinfo=tz_utc)
...
![png](https://cdn.liaoxuefeng.com/cdn/files/attachments/001399415038305edba53df7d784a7fa76c6b7f6526873b000)

这样我们得到`4个数字`作为`索引`，然后查表，获得相应的4个字符，就是编码后的字符串。
...
`struct`模块定义的数据类型可以参考[Python官方文档](https://docs.python.org/3/library/struct.html#format-characters)
...
def bmp_info(data):
    bs = data[0:30]
    data_bytes = struct.unpack('<ccIIIIIIHH', bs)
    return {
        'width': data_bytes[6],
        'height': data_bytes[7],
        'color': data_bytes[9]
...
with open('/path/to/file', 'r') as f:
    f.read()
```
并不是只有 **open()** 函数返回的fp对象才能使用with语句。实际上，任何对象，只要正确实现了上下文管理，就可以用于 **with** 语句。
...
### 练习
请利用SAX编写程序解析Yahoo的XML格式的天气预报，获取天气预报：https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20%3D%202151330&format=xml
